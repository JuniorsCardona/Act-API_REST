...
/**
 * Consultar los inventarios por ID
 */
...
const updateInventario = async (req = request, res = response) => {
    try{
        ...
        const inventarioBD = await Inventario.findOne({ _id: id});       // TODO: VALIDAR QUE EXISTEN Y ESTAN ACTIVOS: ESTADO, USUARIO, MARCA, ...
       if(!inventarioBD){
        return res.status(400).json({
            msj: 'Inventario no existente'
        });
       } 
...
/**
 * Subir foto del equipo al inventario
 */
const uploadImage = async (req = request, res = response) => {
    ...
    if(!req.files || Object.keys(req.files) == 0 || !req.files.foto){
        return res.status(400).json({
            msj: 'No se está subiendo la foto'
        });
    }
...
const getFotoById =  async (req = request, res = response) => {
    ...
    if(!inventarioBD){
        return res.status(400).json({ error: 'Inventario no existente'});
    }
